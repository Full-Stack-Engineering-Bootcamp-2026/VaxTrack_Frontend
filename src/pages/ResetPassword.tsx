    ...
    const onSubmit = async (data: ResetPasswordFormData) => {
        try {
            ...
            if (error instanceof AxiosError) {
                toast.error(error.response?.data?.message || "Failed to update password. The link might be expired.");
            } else {
        ...
        >
            <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
                <div className="space-y-2">
                    <label className="text-sm font-normal text-[#4A4455]">New Password</label>
                    <div className="relative">
                        ...
                        <Input
                            ...
                            {...register("password")}
                            placeholder="••••••••"
                            className="pl-10 pr-10 bg-[#FFF8F4] h-11"
                        />
                        <button
                            type="button"
                ...
                <div className="space-y-2">
                    <label className="text-sm font-normal text-[#4A4455]">Confirm Password</label>
                    <div className="relative">
                        <FiLock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                        <Input
                ...
                <button
                    type="submit"
                    className="w-full mt-2 bg-[#7C3AED] text-white rounded-[12px] py-3.5 text-base font-medium flex items-center justify-center gap-2"
                    disabled={loading}
                >
                    {loading ? "Updating..." : "Update Password"}
                    {!loading && <AiOutlineCheckCircle className="w-5 h-5" />}
                </button>
            </form>
        </PasswordChangeLayout>
    );
